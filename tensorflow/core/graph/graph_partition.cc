...
Status PreprocessGraph(std::unordered_map<const Node*, int> &ready_inputs, Graph* g,
  ...
  for (Node* node : g->nodes()) {
    ...
    if (IsMerge(node)) {
      ...
      for (const Edge* in_edge : node->in_edges()) {
        ...
        if (!in_edge->IsControlEdge()) {
          int call_id;
          TF_RETURN_IF_ERROR(GetNodeAttr(node->attrs(), "call_id", &call_id));
...
void AddPartitionStateMachine(StateMachine& state_machine, GraphDef& main_graphDef,
  ...
  for (const auto &it : sm_graph->nodes) {
    ...
    for (int i = 0; i < sm_node->inputs.size(); ++i) {
      ...
      if (absl::StartsWith(StringPiece(sm_node->inputs[i].src),"Dummy_")) {
...
Status AddNodeToStateMachine(StateMachine& state_machine, string unique_id, Node* node, bool cycle) {
...
Status CallingFunction(Graph* graph, GraphDef& main_graphDef, StateMachine& state_machine, FuncInfo& funcInfo,
  ...
  TF_RETURN_IF_ERROR(AddNodeToStateMachine(state_machine, function_unique_id, call, false));
  ...
  for (const Edge* out_edge : call->out_edges()) {
    if (!out_edge->IsControlEdge()) {
      ...
      TF_RETURN_IF_ERROR(AddNodeToStateMachine(state_machine, function_unique_id, merge, false));
  ...
  while (!ready_nodes.empty()) {
    ...
    if (IsCall(ready_node)) {
      ...
      if (sw == ready_node)
        TF_RETURN_IF_ERROR(AddNodeToStateMachine(state_machine, function_unique_id, ready_node, false));
      parent = sw;
      ...
      int call_id;
      TF_RETURN_IF_ERROR(GetNodeAttr(ready_node->attrs(), "call_id", &call_id));
    ...
    for (const Edge* out_edge : ready_node->out_edges()) {
      ...
      if (src_device != dst_device) {
        if (IsCallSuccessor(ready_node) && IsConstant(out)) {
          ...
          for (string& input : *con_node->mutable_input()) {
            if (absl::StartsWith(StringPiece(input),strings::StrCat("^", ready_node->name()))) {
      ...
      if (ready_inputs[out] == out->in_edges().size()) {
        ...
        if (IsCall(out)) {
          ...
          string frame_name;
          TF_RETURN_IF_ERROR(GetNodeAttr(out->attrs(), "frame_name", &frame_name));
          int call_id;
          ...
          if (funcInfo.funcInputs[frame_name] == calls->size()) {
            ...
            if (sm_graphs.find(unique_id) == sm_graphs.end()) {
              ...
              TF_RETURN_IF_ERROR(CallingFunction(graph, main_graphDef, state_machine, funcInfo, frame_name, call_id, ready_inputs, ready_nodes));
              funcInfo.funcVisitedCounter[frame_name]++;
            } else {
              // Recursive Call (either to the same function or another one (mutual recursion)
              TF_RETURN_IF_ERROR(AddNodeToStateMachine(state_machine, unique_id, (*calls)[0], true));
...
Status AddFunctionStateMachines(const PartitionOptions& opts,
  ...
  TF_RETURN_IF_ERROR(PreprocessGraph(ready_inputs, g, ready_nodes));
      ...
      if (ready_inputs[out] == out->in_edges().size()) {
        ...
        if (IsCall(out)) {
          ...
          if (funcInfo.funcInputs[frame_name] == calls->size()) {
            ...
            TF_RETURN_IF_ERROR(CallingFunction(g, main_graphDef, state_machine, funcInfo, frame_name, call_id, ready_inputs, ready_nodes));
            funcInfo.funcVisitedCounter[frame_name]++;
